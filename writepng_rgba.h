...
template <class T> int getPngColorType()
{
    throw TutorialException("Cannot determine color type");
}

template <> int getPngColorType<agg::rgba8>()
{
    return PNG_COLOR_TYPE_RGBA;
}

template <class RendererBaseType> void writePng(const char* fileName, const RendererBaseType& rBase)
{
    FILE* outFile;
    png_structp pngPtr;
    png_infop infoPtr;
    ...
    png_set_IHDR(pngPtr, infoPtr, width, height, valueSize * 8,
                 getPngColorType<typename RendererBaseType::color_type>(), PNG_INTERLACE_NONE,
